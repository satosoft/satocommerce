        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('tax-class.update',['id' => $data->tax_class_id]) }}" enctype="multipart/form-data" autocomplete="off">
                            @csrf
                            @method('post')

                            <h6 class="heading-small text-muted mb-4">{{ __('Edit Tax Class ') }}</h6>
                            ...
                            <div class="pl-lg-4 row">
                                ...
                                <div class="table-responsive">
                                    <table class="table align-items-center table-flush" id="tbl">
                                        ...
                                        <tbody class="list">
                                        @forelse($data->taxRules as $key => $value)
                                            <tr class="tr_clone">
                                                <td class="budget">
                                                  <select class="form-control" name="tax_rate_id[]" id="tax_rate_id{{$key}}">
                                                    @foreach($taxRates as $taxRate)
                                                      <option value="{{$taxRate->id}}" @if($taxRate->id == $value->tax_rate_id) selected @endif>{{$taxRate->name}}</option>
                                                    @endforeach
                                                  </select>
                                                </td>
                                                <td>
...
@push('js')
    <script>
        var counter = '{{count($data->taxRules)}}';
