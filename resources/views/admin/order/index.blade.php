                                    ...
                                    <td class="">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                ...
                                                <a class="dropdown-item model" type="button"  data-id="{{$value->id}}">Download PDF</a>
                                                <a class="dropdown-item deleteData" type="button"  href="javascript:void(0)" data-url="{{ route('order.delete',['id' => $value->id]) }}">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="download-pdf-form"  action="" method="get">
          <label for="">Select PDF Language</label>
          <select class="form-control" name="pdf_language" class="pdf_language">
            @foreach(getLanguages() as $key=>$language)
              <option value="{{$language->id}}">{{$language->language_name}}</option>
            @endforeach
          </select>
          <input type="hidden" name="order_id" value="" class="order_id_input">
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-primary" onclick="downloadPDF()"> <i class="fa fa-download"></i> Download Now</button>
...
@push('js')
    ...
    <script>
        ...
        function downloadPDF() {
          let languageID = $('.pdf_language :selected').val();
          let orderID =  $('.order_id_input').val()
          var url = '{{ route("order.downloadpdf", ["id" => ":id"]) }}';
          url = url.replace('%3Aid', orderID);
          $('.download-pdf-form').attr('action', url);
