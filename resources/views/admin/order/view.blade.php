...
<div class="container-fluid ">
      ...
      <div class="card p-3">
        ...
        <div class="row">
          <div class="col-md-4">
            ...
            <p class="lefttxt"><button data-toggle="tooltip" title="" class="btn btn-info btn-xs"
                data-original-title="Payment Method"><i class="fa fa-credit-card fa-fw"></i></button> <span
                class="rightTxt">{{$order->payment_method}}</span> </p>
            <p class="lefttxt"><button data-toggle="tooltip" title="" class="btn btn-info btn-xs"
                data-original-title="Shipping Method" aria-describedby="tooltip199408"><i
                  class="fa fa-truck fa-fw"></i></button> <span class="rightTxt">{{$order->shipping_name}}</span> </p>
          </div>
          ...
          <div class="col-md-4">
            ...
            <p class="lefttxt"><button data-toggle="tooltip" title="" class="btn btn-info btn-xs"
                data-original-title="Customer" aria-describedby="tooltip202303"><i
                  class="fa fa-user fa-fw"></i></button> <span class="rightTxt">{{$order->firstname}}
                {{$order->lastname}}</span> </p>
            <p class="lefttxt"><button data-toggle="tooltip" title="" class="btn btn-info btn-xs"
                data-original-title="E-Mail" aria-describedby="tooltip948203"><i
                  class="fa fa-envelope fa-fw"></i></button> <span class="rightTxt">{{$order->email}}</span> </p>
            <p class="lefttxt"><button data-toggle="tooltip" title="" class="btn btn-info btn-xs"
                data-original-title="Telephone" aria-describedby="tooltip751418"><i
                  class="fa fa-phone fa-fw"></i></button> <span class="rightTxt">{{$order->telephone}}</span> </p>
          </div>
          ...
          <div class="col-md-4">
            ...
            <p class="lefttxt">{{$order->orderCountry ? $order->orderCountry->name.', ' :
              null}}{{$order->shipping_postcode}}</p>
          </div>
        </div>
        <hr>
          ...
          <div class="col-md-4 form-group }}">
            <form class="" action="{{route('order.updateStatus',['id' => $order->id])}}" method="post">
              ...
              <select class="form-control" name="order_status_id">
                ...
                <option value={{ $value->id }} @if($value->id == $order->order_status_id) selected="true" @endif>{{
                  $value->name }}</option>
                @endforeach
              </select>
  ...
  overflow-y: scroll;">
              <ul class="list-group list-group-flush">
                ...
                <li class="list-group-item-light mb-1">{{date('d M Y ',strtotime($history->created_at))}} -
                  {{$history->comment}}</li>
                @endforeach
              </ul>
            </div>
        ...
        <div class="row  p-3 table-responsive">
          ...
          <table class="table  table-light ">
            ...
            <tbody>
              @php $subTotal = 0;$grandTotal=0;$totalTax= 0; @endphp
              ...
              <tr>
                ...
                <td><a href=href="{{ route('product.edit',['id' => $product->product_id]) }}">{{$product->name}}</a>
                  <div class="flex-column">
                    ...
                    <div class="mx-3 my-2 d-flex flex-row">
                      ...
                      <div class="p-1 text-bold optionlabel ">{{$option['label']}} @if($option['price'] > 0)
                        (+{{config('settingConfig.config_currency')}}{{$option['price']* $product->quantity}})@endif
                      </div>
                    </div>
                    @endforeach
                    @endif
                ...
                <td>{{config('settingConfig.config_currency')}}{{number_format($product->total,2)}}
                </td>
              </tr>
              ...
              <tr>
                ...
                <td colspan="4"><b>{{config('settingConfig.config_currency')}}{{number_format($subTotal,2) }}</b> </td>
              </tr>
              <tr>
                <td colspan="4" align="right">Shipping Charge</td>
                <td colspan="4"><b>{{config('settingConfig.config_currency')}}{{number_format($order->shipping_charge,2)
                    }}</b> </td>
              </tr>
              @forelse($orderTaxes as $tax)
              <tr>
                <td colspan="4" align="right">{{$tax->tax_name}}</td>
                <td colspan="4"><b>{{config('settingConfig.config_currency')}}{{number_format($tax->tax_amount,2) }}</b>
                </td>
              </tr>
              ...
              <tr>
                ...
                <td colspan="4"><b>{{config('settingConfig.config_currency')}}{{number_format($order->discount,2) }}</b>
                </td>
              </tr>
              @php
              $grandTotal += $subTotal;
              $grandTotal += $order->shipping_charge;
              $grandTotal += $totalTax;
              ...
              <tr>
                ...
                <td colspan="4">
                  <b>{{config('settingConfig.config_currency')}}{{number_format($grandTotal,2)}}</b>
                </td>
              </tr>
            </tbody>
          </table>
