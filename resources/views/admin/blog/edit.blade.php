                        ...
                        <form method="post" action="{{ route('blog.update',['id' => $data->id ]) }}"  autocomplete="off" enctype="multipart/form-data">
                            ...
                            <div class="card-body">
                              ...
                              <div class="tab-content tab-validate" style="margin-top:20px;">
                                @foreach(getLanguages() as $key=>$language)
                                  @php
                                    $find = array_search($language->id, array_column($data->blogMultipleDescription->toArray(), 'language_id'));
                                  @endphp
                                  ...
                                  <h4 class="ml-3 mb-3 text-info"> Enter data in {{$language->language_name}} </h4>
                                   @foreach($data->blogMultipleDescription as $key2=>$langdata)
                                     ...
                                     <div class="pl-lg-4 row">
                                         ...
                                         <div class="col-md-6 form-group{{ $errors->has('multilanguage.*.title') ? ' has-danger' : '' }}">
                                             ...
                                             <input type="text" name="multilanguage[{{$language->id}}][title]" id="input-name" class="form-control blogTitle{{$language->id}} form-control-alternative{{ $errors->has('multilanguage.*.name') ? ' is-invalid' : '' }}" placeholder="{{ __('Enter Blog Title') }}" value="{{ old('title', $langdata->title) }}" autofocus onkeyup="onBlogTitle('{{$language->id}}')">
                                         ...
                                         <div class="pl-lg-4 row">
                                           <div class="col-md-6 form-group{{ $errors->has('short_description') ? ' has-danger' : '' }}">
                                             ...
                                             <textarea name="multilanguage[{{$language->id}}][short_description]" id="short_description" class="form-control form-control-alternative{{ $errors->has('short_description') ? ' is-invalid' : '' }} short_description{{$language->id}}" placeholder="{{ __('Short Description') }}" value="{{ old('short_description', $langdata->short_description) }}" rows="3">
                                               {{ old('short_description', $langdata->short_description) }}
                                             </textarea>
                                       ...
                                       <div class="pl-lg-4 row">
                                         <div class="col-md-8 form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                                             ...
                                             <textarea name="multilanguage[{{$language->id}}][description]"  id="editor{{$language->id}}"  class="ckeditor{{$language->id}} form-control" placeholder="{{ __('Product Description') }}" value="{{ old('description',$langdata->description) }}">{!! old('description',$langdata->description) !!}</textarea>
