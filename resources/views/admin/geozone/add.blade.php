        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('geozone.store') }}" enctype="multipart/form-data" autocomplete="off">
                            @csrf
                            @method('post')

                            <h6 class="heading-small text-muted mb-4">{{ __('Add Geo Zone ') }}</h6>
                            ...
                            <div class="pl-lg-4 row">
                                ...
                                <div class="table-responsive">
                                    <table class="table align-items-center table-flush" id="tbl">
                                        ...
                                        <tr class="tr_clone">
                                            <td class="budget">
                                              <select class="form-control" name="country_id[]" id="country_id0" onchange="fetchStates(this.value,0)">
            ...
            $.ajax({
                ...
                success: function (response) {
                  if(response.data.length > 0) {
                      $('#state_id'+id).empty();
                      let selectHTML = '<option value="0">All States</option>';
                      $.each(response.data, function(key, value) {
                          selectHTML += `<option value="${value.state_id}">${value.name}</option>`
                      });

                      $('#state_id'+id).append(selectHTML);
